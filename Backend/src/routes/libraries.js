...
router.get("/filter", (req, res) => {
  ...
  const filterLibrary = (rating, library_id, authorName, name, ownership) => {
    ...
    let queryString =
      "SELECT * FROM books JOIN libraries ON books.library_id = libraries.id WHERE libraries.id = $1 ";
    ...
    return db
      .query(queryString, values) // Pass author as a parameter to the query
      .then(({ rows }) => {
        console.log(rows);
        res.json(rows);
      })
...
router.get("/users/:id", (req, res) => {
  ...
  FROM 
    LIBRARIES L
    JOIN BOOKS B ON L.ID = B.LIBRARY_ID
  WHERE 
    L.USER_ID = $1
  ORDER BY 
    L.LIBRARY_NAME, B.NAME;`;
  ...
  db.query(queryString, [id])
    .then((result) => {
      if (result.rows.length > 0) {
        ...
        res.status(404).json({ success: false, error: "No libraries found for user" });
      }
    })
    .catch(error => {
      console.error(error);
